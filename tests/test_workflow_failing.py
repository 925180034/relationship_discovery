# tests/test_workflow_failing.py
...
class TestFailingCases:
    ...
    def sample_tables(self):
        """Create test tables fixture with compatible structures"""
        # Source table
    ...
    def mock_components(self, sample_tables):
        """Create mocked components with correct table structures"""
        source_df, target_df = sample_tables
        
        # Mock feature extractor with actual column names
        ...
        # Mock plan generator with correct dimensions
    ...
    def test_few_instances_scenario(self, sample_tables, mock_components,
                                  workflow_config, scenario):
        """Test few instances scenario with parameterization"""
        source_df, target_df = sample_tables
    ...
    def test_error_handling(self, mock_components, workflow_config):
        ...
        workflow_config['experiment']['scenario'] = 'metadata'  # Start with valid
        ...
        workflow.config['experiment']['scenario'] = 'invalid'
        with pytest.raises(ValueError, match="Unknown scenario: invalid"):
            workflow.execute(pd.DataFrame(), pd.DataFrame())

    @pytest.mark.integration
    def test_end_to_end_workflow(self, tmp_path, mock_components):
        """Test end-to-end workflow with mock model"""
        source_df = pd.DataFrame({
            'user_id': range(1, 5),
